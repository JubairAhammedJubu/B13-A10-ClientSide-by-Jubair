...
// Related lesson card with premium lock logic (mirrors MostSavedLessons' TopCard)
function RelatedLessonCard({r, user}) {
  const isLocked = r.accessLevel === "premium" && (!user || !user.isPremium);
  const href = isLocked
    ? user
      ? "/pricing"
      : "/auth/signin"
    ...
    >
      <div className="relative h-32 bg-gray-100 dark:bg-gray-800">
        ...
        {r.accessLevel === "premium" && (
          <div className="absolute top-2 right-2 flex items-center gap-1 px-2 py-0.5 rounded-full bg-black/60 backdrop-blur-sm text-white text-[10px] font-semibold z-10">
...
export default function PublicLessonDetails({lesson, user, canAccess}) {
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/lessons/${lesson._id}/related`,
    )
      .then((res) => res.json())
        ...
        {related.length > 0 && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {related.map((r) => (
                <RelatedLessonCard key={r._id} r={r} user={user} />
              ))}
            </div>
          </motion.div>
        )}
