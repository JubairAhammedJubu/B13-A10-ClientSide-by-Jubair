...
export default async function LessonDetailsPage({params}) {
  ...
  try {
    ...
    const response = await fetch(
      ...
      {
        cache: "no-store",
        // ✅ Pass the token so server knows who is viewing
    ...
    console.log("Lesson:", data?.lesson?.title);
    console.log("Has Access:", data?.hasAccess);
    console.log("creatorIsPremium:", data?.lesson?.creatorIsPremium);

    if (!data?.lesson) return notFound();

    const lesson = data.lesson;
    // ✅ unlock if hasAccess OR creator is premium
    const canAccess = data.hasAccess || lesson.creatorIsPremium === true;

    console.log("Is Locked:", !canAccess);
...
export async function generateMetadata({params}) {
  try {
    ...
    if (!response.ok) return {title: "Lesson Not Found"};
    ...
    return {
      ...
      openGraph: {
        title: lesson.title,
        description: lesson.description?.slice(0, 160),
        images: lesson.image ? [{url: lesson.image}] : [],
      },
    };
  } catch {
    return {title: "Life Lesson"};
  }
}
