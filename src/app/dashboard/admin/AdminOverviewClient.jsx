...
export default function AdminOverviewClient({user}) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [statsRes, lessonsRes, reportsRes] = await Promise.all([
        fetch(`${API}/api/stats`),
        fetch(`${API}/api/lessons?perPage=500`, {headers}),
        fetch(`${API}/api/reports`, {headers}),
      ]);

      const statsData = await statsRes.json();
      // FIX #4b: /api/lessons returns { lessons: [...], total, ... } not a plain array
      ...
      // FIX #4b: parse the nested .lessons array
  ...
  // FIX #4c: lesson schema uses creatorName not authorName
  const contributorMap = {};
  lessons.forEach((l) => {
    const name = l.creatorName;
    if (!name) return;
    if (!contributorMap[name]) {
      contributorMap[name] = {name, count: 0};
    }
  ...
  return (
    <div className="p-4 pt-6 max-w-6xl mx-auto space-y-6">
      ...
      <motion.div
        ...
        className="relative overflow-hidden bg-gradient-to-r from-green-500 via-emerald-500 to-teal-500 rounded-2xl p-6 text-white"
      >
        <div
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-1 overflow-hidden">
                      <motion.div
                        ...
                        className="h-full rounded-full bg-gradient-to-r from-teal-400 to-teal-900"
                      />
                    </div>
                  </div>
